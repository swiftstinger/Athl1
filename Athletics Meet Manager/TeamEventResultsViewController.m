...
#import "TeamEventResultTableViewCell.h"
#import "GEvent.h"
#import "Event.h"
...
- (void)configureView
{
    ...
    if (_detailItem) {
        ...
        _navBar.title =  [NSString stringWithFormat:@"%@: %@",_teamObject.teamName ,_divObject.divName];
    }
...
- (TeamEventResultTableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TeamEventResultTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"eventCell" forIndexPath:indexPath];
...
- (void)configureCell:(TeamEventResultTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
...
NSString* eventScoreString = [self sumResultsForEvent:geventobject InDivision:_divObject InTeam:_teamObject];
          ...
          cell.titleLabel.text = [[geventobject valueForKey:@"gEventName"] description];
    
        cell.scoreLabel.text = [NSString stringWithFormat:@" Score: %@",eventScoreString ] ;


  }

- (NSString*) sumResultsForEvent:(GEvent*) geventobject InDivision: (Division*) divobject InTeam: (Team*) teamObject
{
  ...
  NSFetchRequest *fetchRequest = [[NSFetchRequest alloc]init];
           ...
           NSPredicate *pred1 = [NSPredicate predicateWithFormat:@"(score != NULL)", nil];
            ...
            NSPredicate *pred4 = [NSPredicate predicateWithFormat:@"(event.gEvent == %@)", geventobject];
